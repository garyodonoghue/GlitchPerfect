...
public class HockeyPlayer {
	...
	public void updateIdentiferSprite(SpriteBatch spriteBatch) {
		// this needs to be done sperate as we want other sprites to take
		// priority over it.
	...
	public void updateStick(float delta, Vector2 rotation,
		...
		if (angleStick + 6f >= angleMovement
				&& angleStick - 6f <= angleMovement) {
			// nothing to do
			Vector2 currentForce = this.stick.getLinearVelocity();
			this.stick.applyForce(this.stick.getWorldVector(new Vector2(
					currentForce.x * (-1), currentForce.y * (-1))),
					stickCenter, true);
		...
		this.stick.applyForce(this.stick.getWorldVector(new Vector2(
				forceVector.x, forceVector.y)), stickCenter, true);
	...
	public static Sprite getPlayerSprite(Side side) {
		...
		if (side == Side.Home)
			playerTexture = new Texture(
					Gdx.files.internal("Images/Objects/player_1_noarm.png"));
		else
			// playerTexture = new
			// Texture(Gdx.files.internal("Images/Objects/player_2_noarm.png"));
			playerTexture = null;
